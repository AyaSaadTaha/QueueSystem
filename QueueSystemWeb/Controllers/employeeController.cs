...
namespace QueueSystemWeb.Controllers
{
    ...
    {
        QueueSystemDBEntities3 db = new QueueSystemDBEntities3();
        ...
        public ActionResult GetUnAvailableDates(int service_id)
        {
            ...
            {
                var allTimes = ShowTime(x, service_id);
                if (allTimes.MaxJsonLength == 0)
                {
        ...
        public ActionResult showblock()
        {
            ...
            ViewBag.user_id = new SelectList(db.user, "id");
            var zaft = int.Parse(Session["userid"].ToString());
            var emp_branch = db.Emp_Org_Services.Single(c => c.emp_id == zaft).branch_id;
            var allblockedusers = db.user.Where(x => x.person_id == 4 && x.block == true).ToList();
        ...
        public ActionResult ReplayComplainOrInquire()
        {
            ...
            ViewBag.user_id = new SelectList(db.user, "id");
            var zaft = int.Parse(Session["userid"].ToString());
            int branch_id = (int)db.Emp_Org_Services.SingleOrDefault(c => c.emp_id == zaft).branch_id;
            ...
            if (complain_inquire != null)
            {
                ...
                foreach (Messages item in complain_inquire)
                {
                    ...
                    if (m != null)
                    {
                        ...
                        var replay = " Reply on your " + m.type + " ( " + m.msg + " ) " + " from organization " + " : " + org_name + " is: " + m.replay;
                        //to send mail
